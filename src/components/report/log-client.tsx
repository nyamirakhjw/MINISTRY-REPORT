...
export function LogClient({ memberId, congregationId, month, goalHours, yearLabel, yearSeconds, yearGoalHours, monthlyBreakdown }: {
  ...
  // The year total was computed server-side; swap in the live current-month total so it updates as entries are added.
  const serverCurrentMonth = monthlyBreakdown.find((m) => m.month === month)?.seconds ?? 0;
  const yearTotal = yearSeconds - serverCurrentMonth + log.totalSeconds;
